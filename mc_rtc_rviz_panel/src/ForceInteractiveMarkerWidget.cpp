...
ForceInteractiveMarkerWidget::ForceInteractiveMarkerWidget(
    ...
    bool ro,
    ClientWidget * label)
: ArrowInteractiveMarkerWidget(params, requestId, server, start, start, config, false, !ro, label),
...
{
  Eigen::Vector3d end =  Eigen::Vector3d{feedback->pose.position.x, feedback->pose.position.y, feedback->pose.position.z};
